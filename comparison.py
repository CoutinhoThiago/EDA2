...
def comparison(data):
    ...
    intersection_list = [
        row for row in data
        if row["Color"] == "Red" and row["Color"] == "Green"
    ]
    list_intersection_time = time.time() - start_time

    start_time = time.time()
    intersection_dict = list(set(dict_index["Red"]) & set(dict_index["Green"]))
    dict_intersection_time = time.time() - start_time

    start_time = time.time()
    intersection_bitmap = index.intersect("Color", "Red", "Green")
    bitmap_intersection_time = time.time() - start_time

    start_time = time.time()
    union_list = [
        row for row in data if row["Color"] == "Red" or row["Color"] == "Green"
    ]
    list_union_time = time.time() - start_time

    start_time = time.time()
    union_dict = list(set(dict_index["Red"]) | set(dict_index["Green"]))
    dict_union_time = time.time() - start_time

    start_time = time.time()
    union_bitmap = index.union("Color", "Red", "Green")
    ...
    start_time = time.time()
    difference_dict = list(set(dict_index["Red"]) - set(dict_index["Green"]))
    dict_difference_time = time.time() - start_time

    start_time = time.time()
    difference_bitmap = index.difference("Color", "Red", "Green")
    bitmap_difference_time = time.time() - start_time

    print("\nComparação de desempenho para interseção:")
    print(
        f"Tempo de interseção em lista: {list_intersection_time:.8f} segundos")
    print(
        f"Tempo de interseção em dicionário: {dict_intersection_time:.8f} segundos"
    )
    print(
        f"Tempo de interseção em Bitmap Index: {bitmap_intersection_time:.8f} segundos"
    )

    # Determinar o mais rápido na interseção
    fastest_intersection = min(list_intersection_time, dict_intersection_time,
                               bitmap_intersection_time)
    ...
    print(
        f"Tempo de diferença em dicionário: {dict_difference_time:.8f} segundos"
    )
    print(
        f"Tempo de diferença em Bitmap Index: {bitmap_difference_time:.8f} segundos"
    )

    # Determinar o mais rápido na diferença
    fastest_difference = min(list_difference_time, dict_difference_time,
                             bitmap_difference_time)
